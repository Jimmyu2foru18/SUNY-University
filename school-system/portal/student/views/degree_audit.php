...
$stmt = $pdo->prepare("
    SELECT CS.courseID, E.grade, CS.semesterID
    FROM Enrollment E 
...
foreach ($requirements as $req) {
    ...
    if ($isSatisfied) {
        $completedList[] = ['courseID' => $req['courseID'], 'courseName' => $req['courseName'], 'grade' => $grade, 'credits' => $req['credits']];
    } else {
        $remainingList[] = ['courseID' => $req['courseID'], 'courseName' => $req['courseName'], 'minGrade' => $req['minimumGrade'], 'attemptedGrade' => $grade];
    }
}
    ...
    <main class="portal-main">
        <div class="portal-content">
            ...
            <div class="card border-0 shadow-sm">
                <table class="table table-hover mb-0">
                    <thead class="table-light"><tr><th>Course ID</th><th>Course Name</th><th>Min. Grade</th><th>Attempted</th></tr></thead>
                    <tbody>
                        <?php foreach ($remainingList as $r): ?>
                            <tr>
                                ...
                                <td><?= $r['attemptedGrade'] ? '<span class="badge bg-danger">'.htmlspecialchars($r['attemptedGrade']).'</span>' : '<span class="text-muted">Not Taken</span>'; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
